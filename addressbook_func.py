...
def onAdd(self):
    ...
    if (len(var_street) == 0) or (len(var_city) == 0) or (len(var_state) == 0) or (
            len(var_zip) == 0) or (len(var_fname == 0)) or (len(var_lname == 0)):
...
def onPrevious(self):
    ...
    with conn:
        ...
        if self.bttn_check1 == maxValue:
            ...
            self.bttn_check2 = 0
        elif self.bttn_check3 == 0 or self.bttn_check5 == 0:
            messagebox.showerror("First Row Error", "This is the first record in the database.")
            self.bttn_clicks1 += 1
        else:
            ...
            if maxValue >= self.bttn_clicks1 >= self.bttn_clicks2:
